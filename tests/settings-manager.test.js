...
test('validateSettings mirrors explicit modelService back into native settings fields', () => {
    const { validated } = validateSettings({
        ...
        chatEndpoint: 'https://direct.example.com/ignored',
        kbBaseUrl: 'https://direct-kb.example.com/ignored',
    });
...
test('readSettings fills in missing schema fields from partial settings files', async (t) => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'unistudy-settings-'));
    const settingsPath = path.join(tempRoot, 'settings.json');
    const manager = new SettingsManager(settingsPath);
    t.after(() => fs.remove(tempRoot));
    ...
    await fs.writeJson(settingsPath, {
        userName: 'Partial Settings User',
        enableAgentBubbleTheme: true,
    }, { spaces: 2 });

    const settings = await manager.readSettings();
    assert.equal(settings.userName, 'Partial Settings User');
...
test('readSettings drops removed smooth streaming settings entirely', async (t) => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'unistudy-settings-'));
    const settingsPath = path.join(tempRoot, 'settings.json');
    const manager = new SettingsManager(settingsPath);
    t.after(() => fs.remove(tempRoot));
    ...
    await fs.writeJson(settingsPath, {
        userName: 'Smooth Streaming Removed User',
        enableSmoothStreaming: true,
        smoothStreamIntervalMs: 25,
    }, { spaces: 2 });

    const settings = await manager.readSettings();
    assert.equal('enableSmoothStreaming' in settings, false);

...
function createDynamicIslandController(deps = {}) {
    ...
    function render() {
        ...
        const currentAgentId = session.currentSelectedItem?.id || '';
        ...
        if (el.dynamicIslandShortcutStatus) {
            el.dynamicIslandShortcutStatus.textContent = layout.pomodoroStatus === 'idle'
                ? '专注计时'
        ...
        if (el.dynamicIslandCurrentAgentBtn) {
            el.dynamicIslandCurrentAgentBtn.disabled = !currentAgentId;
            el.dynamicIslandCurrentAgentBtn.classList.toggle('is-disabled', !currentAgentId);
        }
    ...
    function bindEvents() {
        ...
        el.dynamicIslandCurrentAgentBtn?.addEventListener('click', (event) => {
            event.stopPropagation();
            if (!getSessionSlice().currentSelectedItem?.id) {
                return;
            }

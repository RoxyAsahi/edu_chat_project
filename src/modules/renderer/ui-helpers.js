...
<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
    ...
    /**
     * Lite keeps topic actions inline on each topic item, so the old topic context menu is intentionally disabled.
     */
    uiHelperFunctions.showTopicContextMenu = function() {
        console.info('[UI Helper] Topic context menu is disabled in Lite.');
    };
    ...
    uiHelperFunctions.updateAttachmentPreview = function(attachedFiles, attachmentPreviewArea) {
        ...
        attachedFiles.forEach((af, index) => {
            ...
            const fileName = af.originalName || af.name || af.file?.name || '';
            const fileType = af.file?.type || af.type || 'application/octet-stream';
            const previewSrc = af.localPath || af.src || af.internalPath || '';
            prevDiv.title = fileName;
    ...
    /**
     * Helper to get a centrally stored cropped file.
     * @param {string} type The type of avatar ('agent' or 'user').
    ...
    uiHelperFunctions.showPromptDialog = function({
        title = 'Input',
        message = '',
        placeholder = '',
        defaultValue = '',
